...
singleStateLine <- function(state){
  ...
  if(!is.na(zeropoint)){
    ggplot(data=df) + geom_segment(aes(x=lower,xend=zeropoint,y=state,yend=state),colour="firebrick1",size=3) + 
      ...
      geom_text(aes(x = lower, y = state, label = TrumpOrHarris(lower),vjust=2)) +
      geom_text(aes(x = upper, y = state, label = TrumpOrHarris(upper),vjust=2)) +
...
ui <- dashboardPage(skin = "purple",
                    ...
                    dashboardBody(
                      htmlOutput("header"),
                      tabItem(tabName = "Election Plots",box(plotOutput("ECHist",height = "400px"),
...
server <- function(input, output, session) {
  ...
  output$footer <- renderText({
    paste0("<footer>Polling data from 538. State partisan index data from Cook Political Report.</footer>")
  })
}
